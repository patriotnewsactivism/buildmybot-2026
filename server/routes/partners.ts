...
router.get('/clients', async (req, res) => {
  try {
    ...
    const referredClients = partner?.resellerCode
      ? await db.select().from(users).where(eq(users.referredBy, partner.resellerCode))
      : [];
...
router.get('/commissions', async (req, res) => {
  try {
    ...
    const currentTier = RESELLER_TIERS.find(
      (tier) => referredClients.length >= tier.min && referredClients.length <= tier.max
    ) || RESELLER_TIERS[0];

    const whitelabelEnabled = Boolean(partner?.whitelabelEnabled);
    const paidThrough = partner?.whitelabelPaidThrough ? new Date(partner.whitelabelPaidThrough) : null;
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    const paid = referredClients.filter((client) => client.plan !== 'FREE').length;
    const active = referredClients.filter((client) => client.status === 'Active').length;
    const churned = referredClients.filter((client) => client.status === 'Suspended').length;

    const referralClicks = await db
      .select()
      .from(analyticsEvents)
      .where(and(eq(analyticsEvents.eventType, 'referral_click'), eq(analyticsEvents.userId, partnerId)));
...
router.get('/notes', async (req, res) => {
  try {
    ...
    const conditions = [eq(partnerNotes.partnerId, partnerId)];
    if (clientId) {
      conditions.push(eq(partnerNotes.clientId, clientId as string));
    }
...
router.get('/tasks', async (req, res) => {
  try {
    ...
    const conditions = [eq(partnerTasks.partnerId, partnerId)];
    if (clientId) {
      conditions.push(eq(partnerTasks.clientId, clientId as string));
    }
...
router.post('/communications/email', async (req, res) => {
  try {
    ...
    if (!partnerId || !clientId || !subject || !body) {
      return res.status(400).json({ error: 'clientId, subject, and body required' });
    }
