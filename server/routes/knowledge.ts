...
import { db } from '../db';
import { knowledgeSources, bots } from '../../shared/schema';
...
router.post('/scrape/:botId', authenticate, loadOrganizationContext, async (req: Request, res: Response) => {
  try {
...
router.post('/upload/:botId', authenticate, loadOrganizationContext, upload.single('file'), async (req: Request, res: Response) => {
  try {
    const { botId } = req.params;
    const file = req.file;
    const organizationId = (req as any).organizationId;
...
router.get('/sources/:botId', authenticate, loadOrganizationContext, async (req: Request, res: Response) => {
  try {
    const { botId } = req.params;
...
router.delete('/sources/:sourceId', authenticate, loadOrganizationContext, async (req: Request, res: Response) => {
  try {
...
router.get('/search/:botId', authenticate, loadOrganizationContext, async (req: Request, res: Response) => {
  try {
    const { botId } = req.params;
    const { q } = req.query;
...
router.post('/refresh/:sourceId', authenticate, loadOrganizationContext, async (req: Request, res: Response) => {
  try {
    const { sourceId } = req.params;
    ...
    if (s.sourceType === 'url' && s.sourceUrl) {
      WebScraperService.crawlWebsite(s.sourceUrl, s.pagesCrawled || 5, sourceId, s.botId!)
        .catch(console.error);
    }
