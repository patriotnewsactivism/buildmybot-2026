...
router.get('/status/:botId', async (req: Request, res: Response) => {
  try {
    ...
    const status = await channelService.getChannelStatus(botId, organization?.id, user?.id);
    res.json(status);
  } catch (error: any) {
...
router.post('/disable/:botId', async (req: Request, res: Response) => {
  try {
    ...
    await channelService.disableChannel(botId, channel, user.id, organization?.id);
...
router.post('/test/:botId', async (req: Request, res: Response) => {
  try {
    ...
    const result = await channelService.testChannel(botId, channel, organization?.id, user?.id);
    res.json(result);
  } catch (error) {
