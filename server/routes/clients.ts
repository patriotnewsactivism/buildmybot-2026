...
router.get('/overview', async (req, res) => {
  try {
    ...
    const [botCount] = await db
      ...
      .from(bots)
      .where(organizationId ? eq(bots.organizationId, organizationId) : eq(bots.userId, user.id));
    ...
    const conversion = organizationId
      ...
      : { totalConversations: 0, totalLeads: leadCount?.count || 0, conversionRate: 0, averageScore: 0 };
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    if (!user?.organizationId) {
      return res.status(400).json({ error: 'Organization required for analytics' });
    }

    const days = Number(req.query.days || 30);
    const timeSeries = await analyticsService.getTimeSeriesData(user.organizationId, days);
    const conversion = await analyticsService.getConversionMetrics(user.organizationId);
    const botPerformance = await analyticsService.getBotPerformance(user.organizationId);
...
router.post('/onboarding/complete', async (req, res) => {
  try {
    ...
    const preferences = { ...(user.preferences || {}), onboardingComplete: true };
