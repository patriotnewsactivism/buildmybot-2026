...
router.post('/start', async (req, res) => {
  try {
    ...
    const { targetUserId, reason, durationMinutes = 30 } = req.body as {
      targetUserId: string;
      reason: string;
    ...
    if (!isAdmin) {
      const [relation] = await db
        .select()
        .from(partnerClients)
        .where(and(eq(partnerClients.partnerId, actor.id), eq(partnerClients.clientId, targetUserId)));

      if (!relation || (!relation.canImpersonate && relation.accessLevel !== 'full' && relation.accessLevel !== 'manage')) {
...
router.post('/end', async (req, res) => {
  try {
    ...
    await db
      ...
      .where(and(eq(impersonationSessions.id, token), eq(impersonationSessions.actorUserId, actor.id)));
...
router.get('/active', async (req, res) => {
  try {
    ...
    res.json(
      sessions.filter((session) => !session.revokedAt && session.expiresAt.getTime() > Date.now())
    );
  } catch (error) {
