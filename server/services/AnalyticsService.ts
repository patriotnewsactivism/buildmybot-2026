...
export class AnalyticsService {
  ...
  async generateInsights(organizationId: string): Promise<Insight[]> {
    ...
    const currentMetrics = await this.getConversionMetrics(organizationId, oneWeekAgo, now);
    const previousMetrics = await this.getConversionMetrics(organizationId, twoWeeksAgo, oneWeekAgo);

    if (previousMetrics.conversionRate > 0) {
      const changePercent = ((currentMetrics.conversionRate - previousMetrics.conversionRate) / previousMetrics.conversionRate) * 100;
      ...
      if (changePercent > 10) {
        ...
        insights.push({
          ...
          metric: currentMetrics.conversionRate,
          trend: 'down',
          actionable: 'Review and update your bot system prompts for better engagement.',
        });
      }
    }

    if (currentMetrics.totalConversations > previousMetrics.totalConversations * 1.2) {
      insights.push({
        type: 'positive',
        title: 'Traffic Surge',
    ...
    if (peakHours.conversationCount > 0) {
      insights.push({
        ...
        actionable: 'Schedule your marketing campaigns during these peak hours for maximum engagement.',
      });
    }

    const sentimentBreakdown = await this.getSentimentBreakdown(organizationId, oneWeekAgo, now);
    if (sentimentBreakdown.total > 0) {
      const positiveRate = (sentimentBreakdown.positive / sentimentBreakdown.total) * 100;
      if (positiveRate >= 70) {
        insights.push({
          type: 'positive',
        ...
        insights.push({
          ...
          actionable: 'Consider refining your bot responses to be more helpful and friendly.',
        });
      }
    }

    if (currentMetrics.averageScore > 0) {
      const scoreChange = currentMetrics.averageScore - previousMetrics.averageScore;
      if (scoreChange > 5) {
        insights.push({
          type: 'positive',
    ...
    if (insights.length === 0) {
      insights.push({
        ...
        message: 'Keep engaging with customers to unlock more insights about your bot performance.',
      });
    }
  ...
  async getSentimentBreakdown(
    ...
    const conditions: SQL[] = [eq(conversations.organizationId, organizationId)];
  ...
  async getWeekOverWeekGrowth(
    organizationId: string
  ): Promise<{ conversationsGrowth: number; leadsGrowth: number; revenueGrowth: number }> {
    const now = new Date();
    const oneWeekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    const twoWeeksAgo = new Date(now.getTime() - 14 * 24 * 60 * 60 * 1000);
    ...
    return {
      conversationsGrowth: calcGrowth(currentMetrics.totalConversations, previousMetrics.totalConversations),
      leadsGrowth: calcGrowth(currentMetrics.totalLeads, previousMetrics.totalLeads),
      revenueGrowth: 0,
    };
  }
}
