...
export class DocumentProcessorService {
  ...
  static async processDocument(
    ...
    try {
      if (mimeType === 'application/pdf') {
        const pdfResult = await this.extractTextFromPdf(buffer);
        content = pdfResult.text;
        pageCount = pdfResult.pageCount;
        if (content.trim().length < 100) {
          content = await this.extractTextWithOCR(buffer, mimeType);
          ocrUsed = true;
        }
  ...
  static async extractTextWithOCR(buffer: Buffer, mimeType: string): Promise<string> {
  ...
  static chunkDocument(text: string, maxTokens: number = 500): string[] {
    const chunks: string[] = [];
