...
import { organizationBranding, InsertOrganizationBranding } from '../../shared/billing-schema';
import { billingService } from './BillingService';

export class WhitelabelService {
  async getBranding(organizationId: string) {
    const [branding] = await db.select().from(organizationBranding).where(eq(organizationBranding.organizationId, organizationId));
    return branding;
  }

  async createOrUpdateBranding(organizationId: string, data: Partial<InsertOrganizationBranding>) {
    const entitlement = await billingService.checkEntitlement(organizationId, 'white_label');
    ...
    const id = uuid();
    const [branding] = await db.insert(organizationBranding)
  ...
  async updateCustomDomain(organizationId: string, customDomain: string) {
    return this.createOrUpdateBranding(organizationId, { customDomain, domainVerified: false });
  }

  async verifyDomain(organizationId: string) {
    const [branding] = await db.update(organizationBranding)
  ...
  async updateColors(organizationId: string, colors: { primaryColor?: string; secondaryColor?: string; accentColor?: string }) {
    return this.createOrUpdateBranding(organizationId, colors);
  }

  async updateCompanyInfo(organizationId: string, info: { companyName?: string; supportEmail?: string }) {
  ...
  async toggleBuiltWithBadge(organizationId: string, hide: boolean) {
    return this.createOrUpdateBranding(organizationId, { hideBuiltWithBadge: hide });
  }

  async deleteBranding(organizationId: string) {
    await db.delete(organizationBranding).where(eq(organizationBranding.organizationId, organizationId));
  }
}
