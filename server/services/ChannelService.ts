...
export type ChannelType = 'website' | 'whatsapp' | 'messenger' | 'instagram' | 'sms' | 'slack' | 'discord';
...
export class ChannelService {
  ...
  private canAccessBot(bot: any, organizationId?: string, userId?: string): boolean {
  ...
  async getAvailableChannels(): Promise<{ channel: ChannelType; name: string; description: string; requiresSetup: boolean }[]> {
    return [
      {
        channel: 'website',
        name: 'Website Widget',
  ...
  async deployToChannel(
    ...
    switch (channel) {
      case 'website':
        ...
        break;
      case 'whatsapp':
        deployment = await this.deployToWhatsApp(botId, config as WhatsAppConfig);
        break;
      case 'messenger':
        deployment = await this.deployToMessenger(botId, config as MessengerConfig);
        break;
      case 'instagram':
        deployment = await this.deployToInstagram(botId, config as MessengerConfig);
        break;
      case 'sms':
  ...
  private async deployToWebsite(botId: string, config: ChannelConfig): Promise<Deployment> {
  ...
  private async deployToWhatsApp(botId: string, config: WhatsAppConfig): Promise<Deployment> {
  ...
  private async deployToMessenger(botId: string, config: MessengerConfig): Promise<Deployment> {
  ...
  private async deployToInstagram(botId: string, config: MessengerConfig): Promise<Deployment> {
  ...
  private async deployToSMS(botId: string, config: SMSConfig): Promise<Deployment> {
  ...
  private async deployToSlack(botId: string, config: SlackConfig): Promise<Deployment> {
  ...
  private async deployToDiscord(botId: string, config: DiscordConfig): Promise<Deployment> {
  ...
  async getChannelStatus(botId: string, organizationId?: string, userId?: string): Promise<ChannelStatus[]> {
    const [bot] = await db.select().from(bots).where(eq(bots.id, botId));
    if (!bot) {
      throw new Error('Bot not found');
    }
  ...
  async disableChannel(
    ...
    if (!this.canAccessBot(bot, organizationId, userId)) {
      throw new Error('Access denied: Not authorized to disable this bot channel');
    }
  ...
  async testChannel(
    ...
    if (!this.canAccessBot(bot, organizationId, userId)) {
      return { success: false, message: 'Access denied: Not authorized to test this bot' };
    }
