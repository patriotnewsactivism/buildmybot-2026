...
class SystemMetricsService {
  ...
  getSnapshot() {
    ...
    const recent = this.records.filter((record) => record.timestamp >= minuteCutoff);
    const total = recent.length;
    const errors = recent.filter((record) => record.status >= 500).length;
    const avgLatency = total
      ? Math.round(recent.reduce((sum, record) => sum + record.durationMs, 0) / total)
      : 0;

    const activeUsers = Array.from(this.activeUsers.entries()).filter(([, lastSeen]) => lastSeen >= activeCutoff);
