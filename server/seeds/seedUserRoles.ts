...
const USER_ROLES = [
  ...
  { email: 'patriotnewsactivism@gmail.com', role: 'RESELLER', description: 'Reseller' },
  { email: 'news@wtpnews.org', role: 'CLIENT', description: 'Client' },
];

async function ensureOrganization(userId: string, userEmail: string, userRole: string): Promise<string | null> {
  ...
  if (existingOrgs.length > 0) {
    ...
    console.log(`  📁 Found existing organization: ${existingOrgs[0].name} (${orgId})`);
  } else {
    orgId = uuidv4();
...
async function addOrganizationMember(orgId: string, userId: string): Promise<void> {
  ...
  try {
    for (const userConfig of USER_ROLES) {
      ...
      if (existingUsers.length === 0) {
        console.log(`  ⚠️  User ${userConfig.email} does not exist in the database.`);
        console.log(`  📝 Creating user with ${userConfig.description} role...`);

        const userId = uuidv4();
        
        let orgId: string | null = null;
        if (userConfig.role !== 'MasterAdmin' && userConfig.role !== 'ADMIN') {
          orgId = await ensureOrganization(userId, userConfig.email, userConfig.role);
        }
        ...
        await db.insert(users).values({
          ...
          plan: userConfig.role === 'RESELLER' ? 'PROFESSIONAL' : (userConfig.role === 'MasterAdmin' || userConfig.role === 'ADMIN' ? 'ENTERPRISE' : 'FREE'),
          status: 'Active',
          companyName: '',
          organizationId: orgId,
          createdAt: new Date(),
        ...
        console.log(`  ✅ Created user ${userConfig.email} with role: ${userConfig.role}\n`);
      } else {
        ...
        let orgId: string | null = null;
        if (userConfig.role !== 'MasterAdmin' && userConfig.role !== 'ADMIN') {
          orgId = await ensureOrganization(existingUser.id, userConfig.email, userConfig.role);
          if (orgId) {
        ...
        console.log(`  ✅ Updated ${userConfig.email} to ${userConfig.description} (${userConfig.role})\n`);
      }
    }
