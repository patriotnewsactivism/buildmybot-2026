...
import { db } from '../db';
import { users, bots, leads, conversations, organizations, organizationMembers } from '../../shared/schema';
...
export async function migrateToOrganizations(): Promise<MigrationResult> {
  ...
  try {
    ...
    for (const bot of botsWithoutOrg) {
      try {
        ...
        if (!botOwner || !botOwner.organizationId) {
          result.errors.push(`Bot ${bot.id} owner has no organization, skipping`);
          continue;
        }
    ...
    for (const lead of leadsWithoutOrg) {
      try {
        ...
        if (!leadOwner || !leadOwner.organizationId) {
          result.errors.push(`Lead ${lead.id} owner has no organization, skipping`);
          continue;
        }
    ...
    console.log(`Found ${conversationsWithoutOrg.length} conversations to update`);

    for (const conversation of conversationsWithoutOrg) {
      try {
        if (!conversation.userId) {
          result.errors.push(`Conversation ${conversation.id} has no userId, skipping`);
          continue;
        }
        ...
        if (!convOwner || !convOwner.organizationId) {
          result.errors.push(`Conversation ${conversation.id} owner has no organization, skipping`);
          continue;
        }
