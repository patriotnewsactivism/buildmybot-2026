...
export const ResellerDashboard: React.FC<ResellerProps> = ({ user, stats: initialStats }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'clients' | 'payouts' | 'marketing'>('overview');
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-br from-blue-900 to-slate-900 p-6 rounded-xl shadow-lg text-white">
           ...
           <div className="flex items-center gap-2 bg-white/10 p-2 rounded-lg border border-white/20 mb-3">
             ...
             <Copy size={14} className="cursor-pointer hover:text-blue-300" onClick={() => {navigator.clipboard.writeText(referralUrl); alert('Link copied!');}} />
           </div>
      ...
      <div className="bg-gradient-to-r from-amber-50 to-orange-50 p-6 rounded-xl border border-amber-200 shadow-sm">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-3xl font-bold text-amber-600">${referralCredits.credits.toFixed(2)}</p>
            <p className="text-sm text-slate-600 mt-1">Available credit toward your subscription</p>
  ...
  const MarketingTab = () => {
    const [activeSection, setActiveSection] = useState<'playbook' | 'emails' | 'objections' | 'industries' | 'downloads'>('playbook');

    const salesProcess = [
      { step: 1, title: 'Identify the Right Prospects', description: 'Look for businesses with websites that have contact forms, phone numbers, or chat widgets. Service businesses (contractors, dentists, lawyers, realtors) are ideal because they rely heavily on lead capture.' },
      { step: 2, title: 'Research Before You Reach Out', description: 'Spend 2 minutes on their website. Note: Do they have a chat widget? How fast do they respond to inquiries? What are their Google reviews saying? Use this info to personalize your pitch.' },
      { step: 3, title: 'Make First Contact', description: 'Use email or LinkedIn. Keep it short (3-4 sentences max). Focus on THEIR problem, not your product. Ask a question to start a conversation.' },
      { step: 4, title: 'Book the Discovery Call', description: 'Your only goal in initial contact is to get 15 minutes on their calendar. Offer value: "I\'d love to show you how other [industry] businesses are capturing leads 24/7."' },
      { step: 5, title: 'Run the Discovery Call', description: 'Ask questions (see Discovery Questions below). Listen more than you talk. Identify their pain points around lead capture, response time, and missed opportunities.' },
      { step: 6, title: 'Present the Solution', description: 'Show a live demo of BuildMyBot. Use their actual website in the demo if possible. Connect every feature to a problem they mentioned.' },
      { step: 7, title: 'Handle Objections', description: 'See Objection Handling section below. Stay calm, acknowledge their concern, and pivot to value.' },
      { step: 8, title: 'Close the Deal', description: 'Summarize the value, confirm the plan that fits their needs, and walk them through signup. Offer to set up their first bot together.' },
    ];

    const discoveryQuestions = [
      { question: 'How do you currently handle website visitors who have questions outside business hours?', why: 'Reveals if they have a lead capture gap' },
      { question: 'What happens when someone fills out your contact form? How quickly do you respond?', why: 'Most businesses take hours or days - this is your opening' },
      { question: 'How many leads do you think you might be missing because people leave your site without reaching out?', why: 'Gets them thinking about lost revenue' },
      { question: 'What would it mean for your business if you could respond to every website visitor instantly, 24/7?', why: 'Paints the vision of the solution' },
      { question: 'Have you ever lost a customer because a competitor responded faster?', why: 'Creates urgency around speed-to-lead' },
      { question: 'What\'s your average customer worth to your business?', why: 'Sets up the ROI conversation' },
    ];

    const objectionHandling = [
      { 
        objection: '"We already have a contact form."',
        response: 'Contact forms are great, but studies show 70% of visitors leave without filling them out. An AI chatbot engages visitors in real-time, answers their questions, and captures their info conversationally. It\'s like having a salesperson available 24/7.',
        key: 'Highlight the engagement gap'
      },
      { 
        objection: '"AI seems impersonal."',
        response: 'I totally understand that concern. The reality is, today\'s AI is remarkably human-like. Many visitors can\'t tell they\'re chatting with AI. And here\'s the key: an instant, helpful response feels more personal than waiting hours or days for a human reply.',
        key: 'Flip the script on "personal"'
      },
      { 
        objection: '"We tried chatbots before and they didn\'t work."',
        response: 'Most chatbots are rule-based and frustrating. BuildMyBot uses advanced AI (GPT-4) that actually understands context and has real conversations. Would you be open to a quick demo so you can see the difference?',
        key: 'Differentiate from old chatbots'
      },
      { 
        objection: '"It\'s too expensive."',
        response: 'I hear you. Let me ask: what\'s one new customer worth to your business? [Wait for answer]. So if this brings you just one extra customer a month, it pays for itself many times over. Most of our clients see ROI within the first week.',
        key: 'Anchor to customer value'
      },
      { 
        objection: '"I need to think about it."',
        response: 'Absolutely, this is an important decision. What specific concerns are you weighing? [Address those]. Many business owners find it helpful to start with our Starter plan - it\'s low-risk and you can see results within days.',
        key: 'Uncover the real objection'
      },
      { 
        objection: '"I don\'t have time to set this up."',
        response: 'That\'s exactly why we designed this to be plug-and-play. Setup takes about 5 minutes - you paste one line of code on your site. I can even do it with you right now if you\'d like.',
    ...
    const industryPitches = [
      {
        ...
        painPoints: ['Miss calls when on job sites', 'Leads go cold waiting for callbacks', 'After-hours emergencies go unanswered'],
        pitch: 'When you\'re on a roof or under a sink, you can\'t answer the phone. But that homeowner with a leaking pipe? They\'re calling your competitor right now. With BuildMyBot, your AI assistant answers instantly, qualifies the lead, and books the appointment - so you come home to a full calendar instead of voicemails.',
        roi: 'One new job per week at $500 average = $2,000/month. Cost: $99. ROI: 20x'
      },
      {
        industry: 'Dental & Medical Practices',
        painPoints: ['Front desk overwhelmed with calls', 'Patients want to book after hours', 'No-shows and last-minute cancellations'],
        pitch: 'Your front desk is amazing, but they can only answer one call at a time. Meanwhile, patients are booking with the practice that responds first. Our AI handles appointment requests, answers common questions about insurance and services, and even sends reminders to reduce no-shows.',
        roi: 'One extra patient per week at $300 average = $1,200/month. Cost: $99. ROI: 12x'
      },
      {
        industry: 'Real Estate Agents',
        painPoints: ['Buyers want instant responses', 'Leads from Zillow/Realtor go cold', 'Can\'t respond while showing properties'],
        pitch: 'In real estate, the first agent to respond gets the client. Period. When you\'re at a showing and a hot buyer hits your website, our AI engages them instantly, asks about their budget and timeline, and books a call with you. No more "sorry, I went with someone who responded faster."',
        roi: 'One extra transaction per year at $10,000 commission = $833/month. Cost: $99. ROI: 8x'
      },
      {
        industry: 'Law Firms',
        painPoints: ['Initial consultations are time-consuming', 'After-hours inquiries from accident victims', 'Qualifying leads before attorney time'],
        pitch: 'Every minute an injured person waits is a minute they might call another firm. Our AI handles intake 24/7, asks the right qualifying questions, and books consultations only with cases that fit your practice. Your attorneys spend time on clients, not tire-kickers.',
        roi: 'One qualified case per month at $5,000 average = $5,000/month. Cost: $199. ROI: 25x'
      },
      {
        industry: 'Restaurants & Hospitality',
        painPoints: ['Reservation requests pile up', 'Common questions about menu, hours, location', 'Staff too busy during rush'],
        pitch: 'Your staff is slammed during dinner rush - the worst time to answer the phone. Our AI handles reservations, answers questions about your menu and specials, and even helps with catering inquiries. Your team focuses on the guests in front of them.',
...
[Your Name]`
    ...
    const materials = [
      { title: 'Partner Sales Deck', description: 'Professional presentation for pitching to local businesses', icon: Presentation, type: 'PDF', size: '2.4 MB' },
      { title: 'ROI Calculator Spreadsheet', description: 'Customizable calculator to show prospect value', icon: FileText, type: 'XLSX', size: '89 KB' },
      { title: 'Social Media Graphics Pack', description: 'Ready-to-post graphics for all platforms', icon: Image, type: 'ZIP', size: '8.2 MB' },
      { title: 'Case Study Templates', description: 'Document your client success stories', icon: FileText, type: 'DOC', size: '245 KB' },
      { title: 'White-Label Brand Guide', description: 'Guidelines for using BuildMyBot under your brand', icon: FileText, type: 'PDF', size: '1.1 MB' },
      { title: 'Demo Script & Checklist', description: 'Step-by-step guide for running killer demos', icon: FileText, type: 'PDF', size: '156 KB' },
    ];

    return (
      <div className="space-y-6 animate-fade-in">
        <div className="bg-gradient-to-r from-blue-900 to-slate-900 rounded-2xl p-8 text-white">
          <h3 className="text-2xl font-bold mb-2">Sales Playbook & Marketing Materials</h3>
          <p className="text-blue-200 max-w-2xl">
            Everything you need to find prospects, run great sales calls, handle objections, and close deals. 
            This playbook turns new reps into closers.
          </p>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                activeSection === tab.id ? 'bg-blue-900 text-white' : 'text-slate-600 hover:bg-slate-100'
              }`}
            >
              {tab.label}
            </button>
        ...
        {activeSection === 'playbook' && (
          <div className="space-y-6">
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h4 className="font-bold text-lg text-slate-800 mb-6">The 8-Step Sales Process</h4>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h4 className="font-bold text-lg text-slate-800 mb-4">Discovery Questions to Ask</h4>
              <p className="text-slate-500 text-sm mb-6">Use these questions to uncover pain points and build value.</p>
              <div className="space-y-4">
                {discoveryQuestions.map((item, i) => (
                  <div key={i} className="border-l-4 border-blue-600 pl-4 py-2">
                    ...
                    <p className="text-sm text-blue-600 mt-1">Why it works: {item.why}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-6">
              <h4 className="font-bold text-amber-900 mb-3">Golden Rule of Sales</h4>
              <p className="text-amber-800">
                <strong>Listen more than you talk.</strong> On discovery calls, aim for 70% listening, 30% talking. 
                The prospect should feel heard and understood. When you present the solution, connect every feature 
                back to something THEY said was important.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
              <p className="text-blue-800 text-sm">
                <strong>Pro Tip:</strong> Personalize these templates! Mention something specific about their business 
                (a Google review, recent news, their website). Generic emails get ignored.
              </p>
            </div>
            {emailTemplates.map((template, i) => (
              <div key={i} className="bg-white rounded-xl border border-slate-200 p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded font-medium">{template.name}</span>
                    <p className="text-slate-800 font-medium mt-2">Subject: {template.subject}</p>
                  </div>
                  <button 
                    onClick={() => navigator.clipboard.writeText(`Subject: ${template.subject}\n\n${template.body}`)}
                ...
                <pre className="text-sm text-slate-600 whitespace-pre-wrap font-sans bg-slate-50 p-4 rounded-lg">{template.body}</pre>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6">
              <p className="text-emerald-800 text-sm">
                <strong>Remember:</strong> Objections are buying signals! They mean the prospect is engaged and thinking 
                about how this would work for them. Stay calm, acknowledge their concern, and guide them forward.
              </p>
            </div>
              ...
              <div key={i} className="bg-white rounded-xl border border-slate-200 p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h5 className="font-bold text-slate-800">Objection: {item.objection}</h5>
                    ...
                    <p className="text-sm text-blue-600 mt-3 font-medium">Key: {item.key}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="space-y-6">
            <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 mb-6">
              <p className="text-purple-800 text-sm">
                <strong>Industry-Specific Selling:</strong> Different industries have different pain points. 
                Use these tailored pitches to speak directly to what matters most to each prospect.
              </p>
            </div>
            {industryPitches.map((industry, i) => (
              <div key={i} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                  <div>
                    <h6 className="font-semibold text-slate-700 text-sm uppercase tracking-wider mb-2">Pain Points to Mention</h6>
                  ...
                  <div>
                    <h6 className="font-semibold text-slate-700 text-sm uppercase tracking-wider mb-2">Your Pitch</h6>
                    <p className="text-slate-700 bg-blue-50 p-4 rounded-lg italic">"{industry.pitch}"</p>
                  </div>
                  <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                    <h6 className="font-semibold text-emerald-700 text-sm uppercase tracking-wider mb-1">ROI Story</h6>
                    <p className="text-emerald-800 font-medium">{industry.roi}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="space-y-6">
                ...
                <div key={i} className="bg-white rounded-xl border border-slate-200 p-6 hover:shadow-lg transition-all group">
                  <div className="flex items-start justify-between mb-4">
                    <div className="p-3 bg-slate-100 rounded-lg group-hover:bg-blue-100 transition">
                      <item.icon size={24} className="text-slate-600 group-hover:text-blue-900 transition" />
                    </div>
                    <span className="text-xs font-mono bg-slate-100 text-slate-500 px-2 py-1 rounded">{item.type}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button className="flex items-center gap-2 text-sm font-medium text-blue-900 hover:text-blue-700 transition">
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="font-bold text-emerald-900">Need Custom Materials?</h4>
                  <p className="text-sm text-emerald-700 mt-1">
                    Gold and Platinum partners get access to custom-branded materials. 
                    Reach out to your dedicated account manager for personalized sales collateral.
                  </p>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Partner/Reseller Dashboard</h2>
          <p className="text-slate-500">Manage your referrals, payouts, clients, and marketing materials.</p>
        </div>
