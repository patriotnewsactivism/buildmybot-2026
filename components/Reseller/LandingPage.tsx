...
import { Bot, CheckCircle, Globe, ArrowRight, X, Play, LayoutDashboard, MessageSquare, Users, TrendingUp, Flame, Smartphone, Bell, Target, Briefcase, Instagram, DollarSign, Crown, Menu, Gavel, Stethoscope, Home, Landmark, ShoppingBag, Wrench, Car, Utensils, Dumbbell, GraduationCap, Mic, PhoneCall } from 'lucide-react';
...
export const LandingPage: React.FC<LandingProps> = ({ onLogin, onNavigateToPartner, onAdminLogin }) => {
  ...
  const handleDemoSend = async () => {
    ...
    try {
        ...
        setIsTyping(false);
        setChatHistory(prev => [...prev, { role: 'model', text: "Error connecting." }]);
    }
  };

  const openModal = (type: any) => setModalContent(type);
  const closeModal = () => setModalContent(null);
  const InfoModal = () => (modalContent ? <div></div> : null); // Abbreviated for brevity in this file

  const industries = [
    { title: 'Home Services', icon: Wrench, color: 'blue', desc: 'Plumbers, HVAC, Electricians.' },
