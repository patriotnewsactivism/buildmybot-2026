...
interface PartnerRow extends PartnerMetric {
  id: string;
}
...
export const PartnerOversight: React.FC = () => {
  ...
  const activePartners = partners.filter((p) => p.partner.status === 'Active').length;
  ...
  const partnerRows: PartnerRow[] = partners.map((metric) => ({
    ...metric,
  ...
  const partnerColumns: Column<PartnerRow>[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (metric) => (
        <span className="font-semibold text-slate-900">{metric.clientCount}</span>
      ),
    },
    {
      key: 'tier',
      ...
      render: (metric) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          metric.tier === 'Elite' ? 'bg-purple-100 text-purple-800' :
          metric.tier === 'Gold' ? 'bg-yellow-100 text-yellow-800' :
          metric.tier === 'Silver' ? 'bg-slate-100 text-slate-800' :
    ...
    },
    {
      ...
      render: (metric) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          metric.partner.status === 'Active' ? 'bg-green-100 text-green-800' :
          metric.partner.status === 'Pending' ? 'bg-yellow-100 text-yellow-800' :
  ...
  const leaderboardColumns: Column<LeaderboardRow>[] = [
    ...
    },
    {
      ...
      render: (entry) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          entry.tier === 'Elite' ? 'bg-purple-100 text-purple-800' :
          entry.tier === 'Gold' ? 'bg-yellow-100 text-yellow-800' :
          entry.tier === 'Silver' ? 'bg-slate-100 text-slate-800' :
          'bg-blue-100 text-blue-800'
        }`}>
          {entry.tier}
        </span>
