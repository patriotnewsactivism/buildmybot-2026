...
const resourceTypeConfig: Record<string, { icon: React.ElementType; label: string; unit: string; color: string; gradient: string }> = {
  sms_credits: {
    icon: MessageSquare,
    label: 'SMS Credits',
    unit: 'messages',
...
export const UsageCredits: React.FC<UsageCreditsProps> = ({ organizationId }) => {
  ...
  const [activeTab, setActiveTab] = useState<'sms_credits' | 'email_credits' | 'storage_mb'>('sms_credits');
  ...
  const getUsageForType = (resourceType: string): UsageSummary => {
    ...
    return found || { resourceType, total: 0, used: 0, reserved: 0, available: 0 };
  };

  const getPackagesForType = (resourceType: string): CreditPackage[] => {
    switch (resourceType) {
      case 'sms_credits': return smsPackages;
      case 'email_credits': return emailPackages;
      case 'storage_mb': return storagePackages;
      default: return [];
    }
  };
  ...
  const handlePurchase = async (pkg: CreditPackage) => {
    if (!pkg.stripePriceId) {
      alert('This package is not available for purchase yet. Please contact support.');
      return;
    }
  ...
  const handleTopUpAmountChange = (resourceType: string, topUpAmount: number) => {
    setAutoTopUp(prev => ({
      ...prev,
      [resourceType]: {
        ...prev[resourceType],
    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-emerald-400 text-sm font-medium">Usage Credits</span>
              </div>
              ...
              <p className="text-slate-400 mt-2">Manage your messaging and storage resources</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <PremiumCard className="p-6">
          <div className="flex items-start justify-between">
            ...
            <span className="text-xs font-medium text-slate-400 bg-slate-100 px-2 py-1 rounded-full">SMS</span>
          </div>
          <div className="mt-4">
            <div className="text-3xl font-bold text-slate-900">{smsUsage.available.toLocaleString()}</div>
            <div className="text-sm font-medium text-slate-600 mt-1">Credits Remaining</div>
          </div>
          <div className="mt-4">
        ...
        <PremiumCard className="p-6">
          <div className="flex items-start justify-between">
            ...
            <span className="text-xs font-medium text-slate-400 bg-slate-100 px-2 py-1 rounded-full">Email</span>
          </div>
          <div className="mt-4">
            <div className="text-3xl font-bold text-slate-900">{emailUsage.available.toLocaleString()}</div>
            <div className="text-sm font-medium text-slate-600 mt-1">Credits Remaining</div>
          </div>
          <div className="mt-4">
            <div className="flex justify-between text-xs text-slate-500 mb-1">
        ...
        <PremiumCard className="p-6">
          <div className="flex items-start justify-between">
            ...
            <span className="text-xs font-medium text-slate-400 bg-slate-100 px-2 py-1 rounded-full">Storage</span>
          </div>
          <div className="mt-4">
            <div className="text-3xl font-bold text-slate-900">{storageUsage.available.toLocaleString()} <span className="text-lg font-normal text-slate-500">MB</span></div>
            <div className="text-sm font-medium text-slate-600 mt-1">Available Storage</div>
          </div>
          <div className="mt-4">
            <div className="flex justify-between text-xs text-slate-500 mb-1">
      ...
      <PremiumCard className="p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div>
            <h3 className="text-lg font-bold text-slate-900">Purchase Credits</h3>
            <p className="text-sm text-slate-500">Choose a package that fits your needs</p>
          </div>
        ...
        {activePackages.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h4 className="text-lg font-semibold text-slate-900 mt-4">No packages available</h4>
            <p className="text-slate-500 mt-2">{activeConfig.label} packages will appear here once configured.</p>
          </div>
        ) : (
              ...
              return (
                <div
                  ...
                  className={`relative p-6 bg-white rounded-xl border flex flex-col ${
                    pkg.isPopular ? 'border-2 border-orange-500 shadow-lg shadow-orange-500/10' : 'border-slate-200'
                  }`}
                >
                  {pkg.isPopular && (
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className={`p-2 rounded-lg ${pkg.isPopular ? 'bg-orange-100' : 'bg-slate-100'}`}>
                      <Icon size={20} className={pkg.isPopular ? 'text-orange-600' : activeConfig.color} />
                    </div>
                    <div>
                      <h4 className="font-bold text-slate-900">{pkg.name}</h4>
                      <p className="text-xs text-slate-500">{pkg.description || `${pkg.credits} ${activeConfig.unit}`}</p>
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    <div className="flex items-baseline">
                      ...
                      <span className="text-slate-500 ml-1 text-sm">one-time</span>
                    </div>
                    <div className="text-sm text-slate-600 mt-1">
                      {pkg.credits.toLocaleString()} {activeConfig.unit}
                    </div>
                    <div className="text-xs text-slate-400 mt-0.5">
                      ${((pkg.priceCents / 100) / pkg.credits).toFixed(4)}/{activeTab === 'storage_mb' ? 'MB' : 'credit'}
                    </div>
                  </div>
      ...
      <PremiumCard className="p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500">Recent credit consumption and additions</p>
          </div>
          <button
            onClick={fetchData}
        ...
        {usageHistory.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h4 className="text-lg font-semibold text-slate-900">No usage history yet</h4>
            <p className="text-slate-500 mt-2 max-w-md mx-auto">
              When you use SMS, email, or storage credits, your usage history will appear here.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider py-3 px-4">Balance After</th>
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider py-3 px-4">Date</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {usageHistory.map((entry) => {
                  const config = resourceTypeConfig[entry.resourceType] || resourceTypeConfig.sms_credits;
      ...
      <PremiumCard className="p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">Auto Top-up Settings</h3>
            <p className="text-sm text-slate-500">Automatically add credits when balance is low</p>
          </div>
        </div>

        <div className="space-y-6">
          {(['sms_credits', 'email_credits', 'storage_mb'] as const).map((resourceType) => {
            ...
            return (
              <div 
                ...
                className={`p-4 rounded-xl border transition-colors ${
                  settings.enabled ? 'border-orange-200 bg-orange-50' : 'border-slate-200 bg-slate-50'
                }`}
              >
                  ...
                  <div className="mt-4 pt-4 border-t border-orange-200 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleThresholdChange(resourceType, parseInt(e.target.value) || 0)}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => handleTopUpAmountChange(resourceType, parseInt(e.target.value))}
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                      >
                        {topUpAmountOptions.map(amount => (
                          <option key={amount} value={amount}>
                            {amount.toLocaleString()} {resourceType === 'storage_mb' ? 'MB' : 'credits'}
                          </option>
                        ))}
                      </select>
                    </div>
        ...
        <div className="mt-4 flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <span className="text-sm text-blue-800">
            Auto top-up will automatically purchase the selected amount when your balance falls below the threshold.
          </span>
        </div>
      </PremiumCard>
