...
export const VoiceMinutes: React.FC<VoiceMinutesProps> = ({ organizationId }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const voiceUsage = Array.isArray(usageData) 
        ? usageData.find((u: UsageSummary) => u.resourceType === 'voice_minutes')
        : null;
      
      setUsage(voiceUsage || { resourceType: 'voice_minutes', total: 0, used: 0, reserved: 0, available: 0 });
  ...
  const usagePercentage = usage ? Math.min((usage.used / Math.max(usage.total, 1)) * 100, 100) : 0;
    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-emerald-400 text-sm font-medium">Voice Minutes</span>
              </div>
              ...
              <p className="text-slate-400 mt-2">Manage your voice AI calling minutes</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <PremiumCard className="p-6">
          ...
          <div className="mt-4">
            <div className="text-3xl font-bold text-slate-900">{usage?.total?.toLocaleString() || 0}</div>
            <div className="text-sm font-medium text-slate-600 mt-1">Total Minutes</div>
          </div>
        </PremiumCard>
        ...
        <PremiumCard className="p-6">
          <div className="flex items-start justify-between">
            ...
            <span className="text-xs font-medium text-slate-400 bg-slate-100 px-2 py-1 rounded-full">Used</span>
          </div>
          <div className="mt-4">
            <div className="text-3xl font-bold text-slate-900">{usage?.used?.toLocaleString() || 0}</div>
            <div className="text-sm font-medium text-slate-600 mt-1">Minutes Used</div>
          </div>
        </PremiumCard>
        ...
        <PremiumCard className="p-6">
          ...
          <div className="mt-4">
            <div className="text-3xl font-bold text-emerald-600">{usage?.available?.toLocaleString() || 0}</div>
            <div className="text-sm font-medium text-slate-600 mt-1">Minutes Remaining</div>
          </div>
        </PremiumCard>
      </div>
      ...
      <PremiumCard className="p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Track your voice minutes consumption</p>
          </div>
      ...
      <PremiumCard className="p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Automatically add minutes when balance is low</p>
          </div>
          <button
        ...
        {autoTopUp && (
          <div className="mt-4 p-4 bg-orange-50 border border-orange-200 rounded-xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-orange-800">Auto top-up enabled</p>
                <p className="text-xs text-orange-600 mt-1">
                  When your balance drops below 100 minutes, we'll automatically purchase 500 additional minutes.
                </p>
              </div>
            </div>
          </div>
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div>
            <h3 className="text-xl font-bold text-slate-900">Purchase Voice Minutes</h3>
        ...
        {packages.length === 0 ? (
          <PremiumCard className="p-8 text-center">
            ...
            <p className="text-slate-500 mt-2">Voice minute packages will appear here once configured.</p>
          </PremiumCard>
        ) : (
            ...
            {packages.map((pkg) => (
              <PremiumCard
                ...
                className={`relative p-6 flex flex-col ${
                  pkg.isPopular ? 'border-2 border-orange-500 shadow-lg shadow-orange-500/10' : ''
                }`}
              >
                {pkg.isPopular && (
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className={`p-2 rounded-lg ${pkg.isPopular ? 'bg-orange-100' : 'bg-slate-100'}`}>
                    <Phone size={20} className={pkg.isPopular ? 'text-orange-600' : 'text-slate-600'} />
                  </div>
                  <div>
                    <h4 className="font-bold text-slate-900">{pkg.name}</h4>
                    <p className="text-xs text-slate-500">{pkg.description || `${pkg.minutes} minutes package`}</p>
                  </div>
                </div>
      ...
      <PremiumCard className="p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div>
            <h3 className="text-lg font-bold text-slate-900">Recent Call History</h3>
            <p className="text-sm text-slate-500">Your voice AI call activity</p>
          </div>
          <button
            onClick={fetchData}
        ...
        {callLogs.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h4 className="text-lg font-semibold text-slate-900">No calls yet</h4>
            <p className="text-slate-500 mt-2 max-w-md mx-auto">
              When you start making voice AI calls, your call history will appear here with detailed logs.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider py-3 px-4">Direction</th>
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider py-3 px-4">Phone Number</th>
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider py-3 px-4">Duration</th>
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider py-3 px-4">Minutes Used</th>
                ...
                {callLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-sm text-slate-600">
                      {log.direction === 'outbound' ? log.calleeId : log.callerId}
                    </td>
                    <td className="py-3 px-4 text-sm text-slate-600">
