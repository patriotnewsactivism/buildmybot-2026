...
import { 
  Search, Star, Download, Eye, Tag, TrendingUp, Clock, CheckCircle, X, 
...
export const TemplateMarketplace: React.FC<TemplateMarketplaceProps> = ({ 
  ...
  const industries = ['All', ...Array.from(new Set(templates.map(t => t.industry).filter(Boolean))) as string[]];

  let filteredTemplates = templates.filter(t => {
    const matchesSearch = !searchTerm || 
      t.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      t.description?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      t.configuration?.tags?.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const displayTemplates = activeTab === 'browse' ? filteredTemplates : myTemplates;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-xl md:rounded-2xl p-4 md:p-8">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-orange-400 text-sm font-medium">Bot Templates</span>
          </div>
          ...
          <p className="text-slate-400 mt-2">
            {templates.length} templates • {templates.reduce((sum, t) => sum + t.installCount, 0).toLocaleString()}+ installs
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className={`lg:w-72 flex-shrink-0 space-y-4 ${showFilters ? 'block' : 'hidden lg:block'}`}>
        ...
        <div className="flex-1 space-y-4">
          <div className="flex items-center justify-between text-sm text-slate-500">
            <span>Showing {displayTemplates.length} template{displayTemplates.length !== 1 ? 's' : ''}</span>
          </div>
          ...
          {displayTemplates.length === 0 ? (
            <PremiumCard className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-slate-700 mb-2">
                {activeTab === 'my-templates' ? 'No templates installed yet' : 'No templates found'}
              </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {displayTemplates.map(template => (
                <PremiumCard key={template.id} className="flex flex-col h-full group">
                  <div className="p-5 flex-1">
                    ...
                    <h3 className="text-base font-bold text-slate-800 mb-1.5 line-clamp-1">{template.name}</h3>
                    <p className="text-sm text-slate-500 leading-relaxed mb-3 line-clamp-2">{template.description}</p>

                    <div className="flex items-center gap-2 mb-3">
                      <div className="flex items-center gap-1 bg-yellow-50 text-yellow-700 px-2 py-1 rounded-full text-xs font-bold">
                        <Star size={12} fill="currentColor" /> {template.rating.toFixed(1)}
                      </div>
                      <span className="text-xs text-slate-400">•</span>
                      <span className="text-xs text-slate-500">{template.installCount.toLocaleString()} installs</span>
                    </div>
                  ...
                  <div className="p-4 border-t border-slate-100 bg-gradient-to-r from-slate-50 to-white flex items-center justify-between rounded-b-xl">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => template.priceCents === 0 ? handleInstall(template) : handlePurchase(template)}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="sticky top-0 bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-4 flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-sm text-orange-100">
                  {selectedTemplate.category} {selectedTemplate.industry && `• ${selectedTemplate.industry}`}
                </p>
              </div>
              <button
            ...
            <div className="p-6 space-y-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-4 bg-gradient-to-br from-yellow-50 to-amber-50 rounded-xl border border-yellow-100">
                  <div className="flex items-center justify-center gap-1 text-yellow-600 mb-1">
                    ...
                    <span className="text-xl font-bold">{selectedTemplate.rating.toFixed(1)}</span>
                  </div>
                  <div className="text-xs text-slate-500">Rating</div>
                </div>
                <div className="text-center p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
                  <div className="text-xl font-bold text-blue-600 mb-1">{selectedTemplate.installCount.toLocaleString()}</div>
                ...
                <div className="text-center p-4 bg-gradient-to-br from-emerald-50 to-green-50 rounded-xl border border-emerald-100">
                  <div className="text-xl font-bold text-emerald-600 mb-1">
                    {selectedTemplate.priceCents === 0 ? 'Free' : `$${(selectedTemplate.priceCents / 100).toFixed(0)}`}
                  </div>
              ...
              {selectedTemplate.configuration?.features && selectedTemplate.configuration.features.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {selectedTemplate.configuration.features.map((feature, idx) => (
              ...
              {selectedTemplate.configuration?.previewFlow && selectedTemplate.configuration.previewFlow.length > 0 && (
                <div>
                  <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 space-y-3 border border-slate-100">
                    {selectedTemplate.configuration.previewFlow.map((msg, idx) => (
                      <div
                        key={idx}
              ...
              {selectedTemplate.configuration?.tags && selectedTemplate.configuration.tags.length > 0 && (
                <div>
                  <h3 className="font-semibold text-slate-900 mb-2 flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedTemplate.configuration.tags.map(tag => (
                      <span key={tag} className="text-xs bg-slate-100 text-slate-700 px-3 py-1.5 rounded-full">
                        {tag}
                      </span>
                    ))}
                  </div>
            ...
            <div className="sticky bottom-0 bg-white border-t border-slate-200 px-6 py-4 flex items-center justify-between">
              ...
              >
                {purchasing ? (
                  ...
                  <>
                    <Download size={18} />
                    {selectedTemplate.priceCents === 0 ? 'Install Template' : 'Purchase & Install'}
                  </>
                )}
              </button>
            </div>
