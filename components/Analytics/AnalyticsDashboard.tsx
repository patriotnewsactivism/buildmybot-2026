...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ organizationId }) => {
  ...
  const getChartData = () => {
    ...
    return {
      labels: timeSeriesData.map((d: any) => new Date(d.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })),
      datasets: [
        {
          label: 'Conversations',
          data: timeSeriesData.map((d: any) => d.conversations || 0),
        ...
        {
          label: 'Leads',
          data: timeSeriesData.map((d: any) => d.leads || 0),
  ...
  const getBotPerformanceData = () => {
    ...
    return {
      labels: performance.map((p: any) => p.botName),
      datasets: [
        {
          label: 'Conversations',
          data: performance.map((p: any) => p.conversationCount || 0),
          backgroundColor: 'rgba(59, 130, 246, 0.8)',
        },
        {
          label: 'Leads',
          data: performance.map((p: any) => p.leadCount || 0),
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-slate-500">Please select an organization to view analytics</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">Track your bot performance and conversions</p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Bot Name</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Conversations</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Leads</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Conversion Rate</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Avg. Messages</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-slate-200">
                {performance.map((bot: any, index: number) => (
                  <tr key={index} className="hover:bg-slate-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="font-medium text-slate-900">{bot.botName}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2 py-1 text-xs font-medium bg-emerald-100 text-emerald-800 rounded-full">
                        {bot.conversionRate ? `${bot.conversionRate.toFixed(1)}%` : '0%'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-slate-600">
