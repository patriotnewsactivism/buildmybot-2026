...
export const CollaborationHub: React.FC = () => {
  ...
  const handleAddNote = async () => {
    ...
    try {
      const note = await dbService.createPartnerNote({ note: noteInput.trim() });
  ...
  const handleAddTask = async () => {
    ...
    try {
      const task = await dbService.createPartnerTask({ title: taskInput.trim() });
  ...
  const handleToggleTask = async (task: PartnerTask) => {
    ...
    try {
      const updated = await dbService.updatePartnerTask(task.id, { status: nextStatus });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {notes.map((note) => (
            <div key={note.id} className="border border-slate-200 rounded-lg p-3 text-sm text-slate-700">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-slate-900">Onboarding Tasks</h3>
          <button
            onClick={fetchCollaboration}
            className="px-3 py-1.5 bg-slate-100 text-slate-700 rounded-lg text-xs hover:bg-slate-200"
          >
          ...
          {tasks.map((task) => (
            ...
            >
              <div className="flex items-center gap-2">
                <CheckCircle2 size={16} className={task.status === 'completed' ? 'text-emerald-600' : 'text-slate-400'} />
                <span className="font-medium">{task.title}</span>
              </div>
              {task.dueAt && (
                <div className="text-xs text-slate-400 mt-1">Due {new Date(task.dueAt).toLocaleDateString()}</div>
              )}
            </button>
          ))}
