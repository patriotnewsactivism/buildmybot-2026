...
export const LandingPage: React.FC<LandingProps> = ({ onLogin, onNavigateToPartner, onAdminLogin }) => {
  ...
  const handleSend = async (input: string, history: any[], setHistory: any, setTyping: any) => {
  ...
  const handleVoicePreview = async () => {
    ...
    try {
      const response = await fetch("https://api.cartesia.ai/tts/bytes", {
        ...
        body: JSON.stringify({
          model_id: "sonic-2",
          transcript: "Hello! This is Sarah, your AI assistant. I noticed you were looking for information about our services. How can I help you today?",
          voice: {
            mode: "id",
  ...
  const handleMarketingGenerate = async () => {
    ...
    setMarketingResult('');
    try {
      const result = await generateMarketingContent('viral social media post', marketingTopic, 'engaging and persuasive');
  ...
  const projectedRevenue = monthlyLeads * (projectedConversion / 100) * avgDealValue;
  const revenueIncrease = projectedRevenue - currentRevenue;


  const setupSteps = [
    { icon: Globe, title: 'Enter Your Website', description: 'Paste your URL and watch AI learn your entire business in seconds' },
    { icon: Bot, title: 'Customize Your Bot', description: 'Set your brand colors, personality, and lead capture preferences' },
    { icon: FileText, title: 'Copy One Line of Code', description: 'Add a simple script tag to any website - WordPress, Wix, or custom' },
    { icon: TrendingUp, title: 'Watch Leads Roll In', description: 'Your AI starts converting visitors into qualified leads immediately' },
  ];

  const industries = [
    { icon: Wrench, name: 'Home Services', desc: 'HVAC, Plumbing, Roofing' },
    { icon: Home, name: 'Real Estate', desc: 'Agents, Brokers, Property Managers' },
    { icon: Car, name: 'Automotive', desc: 'Dealerships, Repair Shops' },
    { icon: Stethoscope, name: 'Healthcare', desc: 'Clinics, Dentists, Specialists' },
    { icon: Gavel, name: 'Law Firms', desc: 'Personal Injury, Family Law' },
    { icon: Shield, name: 'Bail Bonds', desc: '24/7 Bail Services' },
    { icon: Landmark, name: 'Politicians', desc: 'Campaigns, Constituent Services' },
    { icon: Utensils, name: 'Hospitality', desc: 'Hotels, Restaurants, Events' },
    { icon: Instagram, name: 'Influencers', desc: 'Creators, Personal Brands' },
    { icon: Dumbbell, name: 'Fitness', desc: 'Gyms, Personal Trainers' },
    { icon: GraduationCap, name: 'Education', desc: 'Tutoring, Online Courses' },
    { icon: ShoppingBag, name: 'E-commerce', desc: 'Online Stores, DTC Brands' },
  ];

  const faqs = [
    { q: 'How quickly can I get started?', a: 'Most businesses are up and running in under 5 minutes. Just paste your website URL, customize your bot\'s personality, and copy one line of code to your site. No technical expertise required.' },
    { q: 'Will the AI sound robotic to my customers?', a: 'Not at all. Our AI uses advanced language models and optional voice synthesis (powered by Cartesia) that sounds indistinguishable from a real human. Customers often can\'t tell they\'re talking to an AI.' },
    { q: 'What happens if the AI can\'t answer a question?', a: 'The AI is trained to gracefully hand off to a human when needed. It will capture the lead\'s information and notify you immediately, so no opportunity is ever lost.' },
    { q: 'Can I integrate with my existing CRM and tools?', a: 'Yes! We integrate with popular CRMs like Salesforce, HubSpot, and Zoho, plus calendar tools like Calendly and Google Calendar. API access is available on Professional plans and above.' },
    { q: 'Is my data secure?', a: 'Absolutely. We use enterprise-grade encryption, SOC 2 compliance, and never train our models on your data. Your business information stays yours.' },
    { q: 'What if I need to cancel?', a: 'Cancel anytime with no questions asked. We offer a 14-day money-back guarantee on all paid plans. Your data can be exported or deleted upon request.' },
  ];
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-blue-400 text-sm font-medium">Ultra-Realistic Preview</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4">
          ...
          >
            {isVoiceActive ? <Loader className="animate-spin" size={20} /> : <Play size={20} />}
    ...
    return (
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden flex flex-col h-[400px] sm:h-[500px] lg:h-[600px] w-full max-w-md mx-auto">
        <div className="bg-slate-900 p-4 text-white flex items-center gap-3">
          <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center font-bold">FE</div>
          <div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50" ref={scrollRef}>
          {embedHistory.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] px-4 py-2 rounded-xl text-sm ${msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-white border border-slate-200'}`}>
                {msg.text}
              </div>
            </div>
          ))}
          {embedTyping && <div className="text-slate-400 text-xs italic">Typing...</div>}
        </div>
        <div className="p-4 border-t flex gap-2">
          <input 
            value={embedInput}
            onChange={(e) => setEmbedInput(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && (handleSend(embedInput, embedHistory, setEmbedHistory, setEmbedTyping), setEmbedInput(''))}
          ...
          />
          <button 
            onClick={() => { handleSend(embedInput, embedHistory, setEmbedHistory, setEmbedTyping); setEmbedInput(''); }}
        ...
        {isHoverOpen && (
          <div className="w-[calc(100vw-3rem)] sm:w-80 h-[400px] sm:h-[500px] max-w-sm bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5">
            <div className="bg-blue-900 p-4 text-white flex justify-between items-center">
              ...
              <button onClick={() => setIsHoverOpen(false)}><X size={20} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50" ref={chatScrollRef}>
              ...
              {isTyping && <div className="text-slate-400 text-xs italic">Typing...</div>}
            </div>
            <div className="p-4 border-t flex gap-2">
              <input 
                value={chatInput}
                onChange={(e) => setChatInput(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && (handleSend(chatInput, chatHistory, setChatHistory, setIsTyping), setChatInput(''))}
                className="flex-1 bg-slate-100 rounded-lg px-4 py-2 text-sm"
                placeholder="Ask me anything..."
              />
              <button onClick={() => { handleSend(chatInput, chatHistory, setChatHistory, setIsTyping); setChatInput(''); }} className="text-blue-900"><Send size={20} /></button>
            </div>
          </div>
        )}
        <button 
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-12 py-16 sm:py-24 space-y-24 md:space-y-40">
        ...
        <section className="text-center space-y-10 pt-8">
          ...
          <p className="text-lg sm:text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed">
            Your AI sales agent works 24/7, responds in seconds, and never takes a day off. 
            Convert website visitors into paying customers while you sleep.
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4">
            <button onClick={onLogin} className="w-full sm:w-auto bg-blue-700 text-white px-10 py-4 rounded-2xl font-bold text-lg hover:bg-blue-800 transition-all shadow-xl shadow-blue-700/30 hover:shadow-2xl hover:shadow-blue-700/40 hover:-translate-y-0.5 flex items-center justify-center gap-3">
              Start Free <ArrowRight size={20} />
            </button>
            <a href="#demo" className="w-full sm:w-auto bg-white text-slate-700 border-2 border-slate-200 px-10 py-4 rounded-2xl font-bold text-lg hover:border-blue-300 hover:text-blue-700 transition-all flex items-center justify-center gap-3 shadow-sm">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 pt-6 text-sm text-slate-500">
            ...
            <span className="flex items-center gap-2"><CheckCircle size={16} className="text-emerald-500" /> 5-minute setup</span>
            ...
            <div className="space-y-8 max-w-xl">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  Monthly Website Leads: <span className="text-blue-900 font-bold">{monthlyLeads}</span>
                </label>
                <input 
                  type="range" min="1" max="1000" value={monthlyLeads}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  Average Deal Value: <span className="text-blue-900 font-bold">${avgDealValue.toLocaleString()}</span>
                </label>
                <input 
                  type="range" min="100" max="50000" step="100" value={avgDealValue}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  Current Conversion Rate: <span className="text-blue-900 font-bold">{currentConversion}%</span>
                </label>
                <input 
                  type="range" min="1" max="50" value={currentConversion}
            ...
            <div className="bg-gradient-to-br from-blue-900 to-blue-800 rounded-2xl p-8 text-white">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center pb-4 border-b border-blue-700">
                  ...
                  <span className="text-2xl font-bold text-emerald-400">${projectedRevenue.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center pt-2">
                  <span className="text-xl font-medium">Additional Revenue</span>
                  <span className="text-3xl font-extrabold text-emerald-400">+${revenueIncrease.toLocaleString()}</span>
                </div>
              </div>
              <button onClick={onLogin} className="w-full mt-6 bg-white text-blue-900 py-3 rounded-xl font-bold hover:bg-blue-50 transition">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-8 overflow-hidden">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-bold text-red-900">WITHOUT BuildMyBot</h3>
            </div>
            <ul className="space-y-4">
              {[
                ...
                <li key={i} className="flex items-start gap-3 text-red-800 leading-relaxed">
          ...
          <div className="bg-emerald-50 border-2 border-emerald-200 rounded-2xl p-8 overflow-hidden">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-bold text-emerald-900">WITH BuildMyBot</h3>
            </div>
            <ul className="space-y-4">
              {[
                ...
                <li key={i} className="flex items-start gap-3 text-emerald-800 leading-relaxed">
        ...
        <section id="voice" className="space-y-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
            <div className="bg-slate-100 p-6 rounded-xl text-center overflow-hidden">
              ...
              <p className="text-slate-600 text-sm leading-relaxed">Never miss another call, even at 3am</p>
            </div>
            <div className="bg-slate-100 p-6 rounded-xl text-center overflow-hidden">
              <Mic className="mx-auto mb-3 text-blue-900" size={28} />
              <h4 className="font-bold mb-1">Human-Like Voice</h4>
              <p className="text-slate-600 text-sm leading-relaxed">Callers won't know it's AI</p>
            </div>
            <div className="bg-slate-100 p-6 rounded-xl text-center overflow-hidden">
              <Target className="mx-auto mb-3 text-blue-900" size={28} />
              <h4 className="font-bold mb-1">Lead Qualification</h4>
              <p className="text-slate-600 text-sm leading-relaxed">Ask intake questions, book appointments</p>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            {industries.map((ind, i) => (
              <div key={i} className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 hover:border-blue-500 hover:shadow-lg transition-all text-center group cursor-pointer overflow-hidden">
                <ind.icon className="mx-auto mb-3 text-slate-400 group-hover:text-blue-900 transition" size={32} />
                <h4 className="font-bold text-sm sm:text-base mb-1">{ind.name}</h4>
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-slate-300 text-lg mb-8 leading-relaxed">
                White-label our technology under your brand. Sell AI chatbots to local businesses while we handle all the infrastructure. Zero coding required.
              </p>
              <ul className="space-y-3 mb-8">
                <li className="flex items-center gap-3 text-slate-200">
                  <CheckCircle size={20} className="text-emerald-400" /> Earn 20%-50% recurring commissions (grow your way up)
                </li>
                <li className="flex items-center gap-3 text-slate-200">
                  <CheckCircle size={20} className="text-emerald-400" /> Or pay $499 once for instant 50% Whitelabel access
                </li>
                <li className="flex items-center gap-3 text-slate-200">
                  <CheckCircle size={20} className="text-emerald-400" /> Your brand, your pricing, your clients
                </li>
              </ul>
              <button 
          ...
          <div className="space-y-6">
            ...
            <ul className="space-y-4">
              <li className="flex items-center gap-3 text-slate-700">
                <CheckCircle className="text-emerald-500" size={20} /> Customizable hover widgets for lead gen
              </li>
              <li className="flex items-center gap-3 text-slate-700">
                <CheckCircle className="text-emerald-500" size={20} /> Fixed embeds for deep knowledge base search
              </li>
              <li className="flex items-center gap-3 text-slate-700">
                <CheckCircle className="text-emerald-500" size={20} /> Shared knowledge across all interfaces
              </li>
            </ul>
          </div>
          <FixedEmbedChat />
              ...
              return (
                ...
                >
                  ...
                  <div className="text-sm text-slate-600 mb-4">
                    {plan.bots >= 9999 ? 'Unlimited' : plan.bots} bot{plan.bots !== 1 ? 's' : ''} · {plan.conversations.toLocaleString()} convos
                  </div>
        ...
        <section className="bg-gradient-to-br from-blue-900 to-blue-800 rounded-3xl p-12 md:p-16 text-white text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <button onClick={onLogin} className="w-full sm:w-auto bg-white text-blue-900 px-8 py-4 rounded-xl font-bold text-lg hover:bg-blue-50 transition flex items-center justify-center gap-2">
              Start Free Now <ArrowRight size={20} />
            </button>
            <a href="#demo" className="w-full sm:w-auto border-2 border-white text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-white/10 transition flex items-center justify-center gap-2">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-6 text-blue-200 text-sm">
            ...
            <span className="flex items-center gap-2"><CheckCircle size={16} /> 5-minute setup</span>
      ...
      <footer className="bg-slate-900 text-slate-400 py-20 px-6">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-12">
          <div>
            ...
            <p className="text-sm">The AI workforce that never sleeps. Convert more leads, close more deals, grow your business.</p>
          </div>
          <div>
