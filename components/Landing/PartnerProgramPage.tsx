...
import { CheckCircle, DollarSign, TrendingUp, Users, ArrowRight, Calculator, Shield, Zap, Globe, ArrowLeft, LayoutDashboard, Crown, Rocket } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <section className="py-20 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
            ...
            <div className="bg-slate-50 rounded-2xl p-8 border border-slate-200">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Growth Path</h3>
                  <p className="text-slate-500 text-sm">Free to start, earn your way up</p>
                </div>
              </div>
              
              <div className="text-3xl font-extrabold text-slate-900 mb-2">$0 <span className="text-lg font-normal text-slate-500">to start</span></div>
              <p className="text-slate-600 mb-6">Start earning 20% immediately. Grow your client base to unlock higher commission tiers—up to 50%.</p>
              
              <div className="space-y-3 mb-6">
                <div className="flex items-center justify-between py-2 border-b border-slate-200">
                  <span className="text-slate-700">0-49 clients</span>
                  <span className="font-bold text-blue-700">20% commission</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-slate-200">
                  <span className="text-slate-700">50-149 clients</span>
                  <span className="font-bold text-blue-700">30% commission</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-slate-200">
                  <span className="text-slate-700">150-249 clients</span>
                  <span className="font-bold text-blue-700">40% commission</span>
                </div>
                <div className="flex items-center justify-between py-2">
                  <span className="text-slate-700">250+ clients</span>
                  <span className="font-bold text-emerald-600">50% commission</span>
                </div>
              </div>

              <ul className="space-y-2 text-sm text-slate-600 mb-6">
                <li className="flex items-center gap-2"><CheckCircle size={14} className="text-emerald-600"/> No upfront investment</li>
                <li className="flex items-center gap-2"><CheckCircle size={14} className="text-emerald-600"/> Zero risk to get started</li>
                <li className="flex items-center gap-2"><CheckCircle size={14} className="text-emerald-600"/> Grow at your own pace</li>
              </ul>

              <button onClick={onSignup} className="w-full py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition">
                Start Free
              </button>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-900 to-slate-900 rounded-2xl p-8 border border-blue-700 relative overflow-hidden">
              ...
              <div className="bg-white/10 rounded-xl p-4 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-3xl font-extrabold text-emerald-400">50%</span>
                </div>
                <p className="text-blue-300 text-sm mt-1">From day one, on every client</p>
              </div>

              <ul className="space-y-2 text-sm text-blue-100 mb-6">
                {WHITELABEL_FEE.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center gap-2"><CheckCircle size={14} className="text-emerald-400"/> {feature}</li>
                ))}
              </ul>

              <button onClick={onSignup} className="w-full py-3 bg-white text-slate-900 rounded-xl font-bold hover:bg-blue-50 transition flex items-center justify-center gap-2">
          ...
          <div className="text-center mb-8">
            <h3 className="text-xl font-bold text-slate-700">Growth Path Tier Details</h3>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {RESELLER_TIERS.map((tier, idx) => (
              <div key={idx} className={`p-6 rounded-xl border text-center ${
                tier.label === 'Platinum' ? 'bg-slate-900 text-white border-slate-700' : 'bg-white border-slate-200'
              }`}>
                <h4 className={`font-bold text-sm mb-1 ${tier.label === 'Platinum' ? 'text-blue-400' : 'text-slate-500'}`}>{tier.label}</h4>
                <div className="text-2xl font-extrabold mb-1">{(tier.commission * 100)}%</div>
                <div className={`text-xs ${tier.label === 'Platinum' ? 'text-slate-400' : 'text-slate-500'}`}>
                  {tier.max > 100000 ? `${tier.min}+` : `${tier.min}-${tier.max}`} clients
                </div>
              </div>
            ))}
          </div>
      ...
      <footer className="bg-slate-50 border-t border-slate-200 py-12 px-6 text-center">
         ...
         <p className="text-slate-500 mb-8">Join our growing network of partner agencies.</p>
