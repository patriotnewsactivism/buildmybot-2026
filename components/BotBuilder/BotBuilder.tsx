...
export const BotBuilder: React.FC<BotBuilderProps> = ({ bots, onSave, customDomain, onLeadDetected }) => {
  ...
  useEffect(() => {
    if (activeBot.id === 'new' || selectedBotId === 'new') {
      ...
      const savedBot = bots.find(b => b.name === activeBot.name && b.id !== 'new');
      if (savedBot) {
      ...
      if (updatedBot && JSON.stringify(updatedBot) !== JSON.stringify(activeBot)) {
  ...
  const handleFileUpload = async (files: FileList | null) => {
    if (!files || files.length === 0 || !activeBot.id || activeBot.id === 'new') {
      if (activeBot.id === 'new') {
    ...
    try {
      const newDoc = await dbService.uploadBotDocument(activeBot.id, file, setUploadProgress);
      if (newDoc) {
...
<script src="https://${displayDomain}/embed.js" async></script>`;
  ...
  return (
    <div className="h-[calc(100vh-6rem)] flex gap-4 md:gap-6 animate-fade-in overflow-x-hidden">
      {/* Sidebar List */}
      ...
      <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
         ...
         <div className="min-h-16 border-b border-slate-100 px-4 md:px-6 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-white">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
               ...
               >
                 <Save size={18} /> <span className="hidden sm:inline">Save Changes</span><span className="sm:hidden">Save</span>
               </button>
            </div>
         </div>

         {/* Tabs */}
         <div className="border-b border-slate-200 bg-slate-50 px-4 md:px-6 flex gap-4 md:gap-6 overflow-x-auto">
            {[
         ...
         <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50/50">
            {activeTab === 'config' && (
                <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                        <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                           <MessageSquare size={18} className="text-blue-900" /> Chatbot Display Type
                        </h3>
                        <p className="text-sm text-slate-600 mb-4">Choose how the chatbot appears on your website.</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           <button
                              onClick={() => setActiveBot({...activeBot, embedType: 'hover'})}
                           ...
                           >
                              <div className="flex items-start gap-3">
                                 ...
                                 <div>
                                    <div className="font-semibold text-slate-900">Floating Bubble</div>
                                    <p className="text-xs text-slate-500 mt-1">A small chat bubble appears in the corner. Visitors click to open the chat window.</p>
                                 </div>
                              </div>
                           </button>
                           <button
                              onClick={() => setActiveBot({...activeBot, embedType: 'fixed'})}
                              className={`p-4 rounded-xl border-2 transition-all text-left ${
                           ...
                           >
                              <div className="flex items-start gap-3">
                                 ...
                                 <div>
                                    <div className="font-semibold text-slate-900">Fixed Embed</div>
                                    <p className="text-xs text-slate-500 mt-1">Chat window is always visible on the page. Great for dedicated support pages.</p>
                                 </div>
                              </div>
                           </button>
                        </div>
                ...
                <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                    <div className="space-y-6">
                        ...
                        {activeBot.id === 'new' && (
                          <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-xl flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm">Please save your bot before copying the embed code or share link. This ensures visitors can access your chatbot.</p>
                            </div>
                          </div>
                        )}
                        ...
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                           ...
                           <div className={`bg-slate-900 text-slate-300 p-4 rounded-lg font-mono text-xs overflow-x-auto relative group ${activeBot.id === 'new' ? 'opacity-50' : ''}`}>
                        ...
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                           ...
                           <div className={`flex items-center gap-2 bg-slate-50 p-2 rounded-lg border border-slate-200 mb-4 ${activeBot.id === 'new' ? 'opacity-50' : ''}`}>
                              <input 
                                readOnly 
                                value={activeBot.id === 'new' ? 'Save bot to generate share link...' : shareLink}
