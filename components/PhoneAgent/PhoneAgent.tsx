...
const cartesiaVoices = [
  ...
  { id: 'f786b574-daa5-4673-aa0c-cbe3e8534c02', name: 'Sarah', description: 'Warm, conversational' },
  { id: '79a125e8-cd45-4c13-8a67-188112f4dd22', name: 'British Lady', description: 'British accent' },
  { id: '421b3369-f63f-4b03-8980-37a44df1d4e8', name: 'Confident Man', description: 'Professional male' },
  { id: '87748186-23bb-4158-a1eb-332911b0b708', name: 'Friendly Man', description: 'Approachable male' },
  { id: 'c2ac25f9-efd4-4f5d-8545-4e4212d7a5e5', name: 'Storyteller', description: 'Engaging narrator' },
];
...
export const PhoneAgent: React.FC<PhoneAgentProps> = ({ user, onUpdate }) => {
  ...
  const [activeTab, setActiveTab] = useState<'setup' | 'config'>(cartesiaApiKey ? 'config' : 'setup');
  ...
  const playVoicePreview = async (voiceId: string) => {
    ...
    try {
      const response = await fetch('https://api.cartesia.ai/tts/bytes', {
        ...
        body: JSON.stringify({
          model_id: 'sonic-2',
          transcript: 'Hello! This is a preview of how I sound. How can I help you today?',
       ...
       <div className="border-b border-slate-200 flex gap-6">
          <button
            ...
            className={`pb-3 text-sm font-medium border-b-2 transition ${
              activeTab === 'setup' ? 'border-blue-900 text-blue-900' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
          >
          ...
          <button
            ...
            className={`pb-3 text-sm font-medium border-b-2 transition ${
              activeTab === 'config' ? 'border-blue-900 text-blue-900' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
          >
         ...
         <div className="space-y-6 animate-fade-in">
           ...
           <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-100">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
               <div className="bg-white p-4 rounded-lg border border-blue-100">
                 ...
                 <h4 className="font-semibold text-slate-800 mb-1">Get Your API Key</h4>
                 <p className="text-sm text-slate-600">Sign up at Cartesia.ai and get your API key from the dashboard.</p>
               </div>
               <div className="bg-white p-4 rounded-lg border border-blue-100">
                 <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-700 font-bold mb-3">2</div>
                 <h4 className="font-semibold text-slate-800 mb-1">Enter Key Below</h4>
                 <p className="text-sm text-slate-600">Paste your API key in the field below to connect your account.</p>
               </div>
               <div className="bg-white p-4 rounded-lg border border-blue-100">
                 <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-700 font-bold mb-3">3</div>
                 <h4 className="font-semibold text-slate-800 mb-1">Choose a Voice</h4>
                 <p className="text-sm text-slate-600">Preview voices, customize your greeting, and you're ready!</p>
               </div>
             </div>
           </div>
           ...
           <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
             <div className="flex items-center justify-between mb-4">
               <div className="flex items-center gap-2">
                 <div className="p-2 bg-amber-50 text-amber-600 rounded-lg"><Key size={18} /></div>
             ...
             <div className="space-y-4">
               <div>
                 ...
                 <p className="text-xs text-slate-500 mt-2">
                   Your API key is stored securely and used only for voice generation.
                 </p>
               </div>
                 ...
                 <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3">
                   ...
                   <div>
                     ...
                     <p className="text-sm text-amber-700 mt-1">
                       You need a Cartesia API key to use voice features. Get one free at cartesia.ai
                     </p>
                   </div>
                 </div>
               )}
           ...
           <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
             <div className="flex items-center gap-2 mb-4">
               <div className="p-2 bg-purple-50 text-purple-600 rounded-lg"><ExternalLink size={18} /></div>
               <h3 className="font-bold text-slate-800">Delegated Account (Optional)</h3>
             </div>
             <p className="text-sm text-slate-600 mb-4">
               If you're working with a partner or have a delegated Cartesia account, enter your connection details here.
             </p>
             <div className="space-y-4">
               <div>
                 <label className="block text-sm font-medium text-slate-700 mb-2">Delegation Link (Optional)</label>
                 <input 
                   type="url"
                 ...
                 <p className="text-xs text-slate-500 mt-1">
                   If your account manager provided a delegation link, paste it here.
                 </p>
               </div>
             </div>
           </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
            ...
            <div className="md:col-span-2 space-y-6">
               <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div>
                       <label className="block text-sm font-medium text-slate-700 mb-2">What your AI will say when answering</label>
                       <textarea 
                         value={introMessage}
               ...
               <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-xs text-slate-500 ml-auto">Powered by Cartesia AI</span>
                  </div>
                    ...
                    {cartesiaVoices.map((v) => (
                      <div 
                        ...
                        className={`p-3 rounded-lg border cursor-pointer hover:border-blue-300 transition ${
                          voice === v.id ? 'border-blue-900 bg-blue-50' : 'border-slate-200 bg-white'
                        }`}
                      >
                        <div className="flex justify-between items-start">
                          ...
                          >
                            {isPlayingPreview === v.id ? <Loader size={16} className="animate-spin" /> : <Volume2 size={16} />}
                          </button>
                        </div>
                      </div>
                    ))}
               ...
               <div className="bg-slate-900 text-white p-6 rounded-xl shadow-lg relative overflow-hidden">
                  <div className="relative z-10 text-center">
                     ...
                     <div className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center mb-4 transition-all duration-500 ${isSimulating ? 'bg-red-500 animate-pulse' : 'bg-emerald-500 hover:scale-105 cursor-pointer'}`} onClick={isSimulating ? endSimulation : startSimulation}>
                        {isSimulating ? <Phone size={32} className="rotate-135" /> : <PlayCircle size={32} />}
                     </div>
                     
                     <p className="text-sm font-medium">{simulationStatus}</p>
                     <p className="text-xs text-slate-400 mt-2">Click to hear your greeting</p>
                  </div>
               </div>
