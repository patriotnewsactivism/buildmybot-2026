...
const AVAILABLE_SCOPES = [
  ...
  { id: 'bots:write', label: 'Write Bots', description: 'Create and update bots' },
  ...
  { id: 'chat:read', label: 'Read Conversations', description: 'View chat logs' },
  { id: 'chat:write', label: 'Send Messages', description: 'Send messages via API' },
  { id: 'knowledge:read', label: 'Read Knowledge Base', description: 'View knowledge base content' },
  { id: 'knowledge:write', label: 'Write Knowledge Base', description: 'Update knowledge base' },
...
const StatCard: React.FC<{ icon: React.ElementType; label: string; value: string | number; trend?: 'up' | 'down' | 'neutral'; trendValue?: string }> = ({ icon: Icon, label, value, trend, trendValue }) => (
  <PremiumCard className="p-4 md:p-6">
    <div className="flex items-start justify-between">
      ...
      {trend && trendValue && (
        <div className={`flex items-center gap-1 text-xs font-medium ${trend === 'up' ? 'text-emerald-600' : trend === 'down' ? 'text-red-600' : 'text-slate-500'}`}>
...
export const ApiKeyManager: React.FC<ApiKeyManagerProps> = ({ organizationId, userId }) => {
  ...
  const createApiKey = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNotification({ type: 'error', message: error.error || 'Failed to create API key' });
  ...
  const revokeApiKey = async (apiKeyId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        setApiKeys(apiKeys.map(k => k.id === apiKeyId ? { ...k, isActive: false } : k));
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-emerald-400 text-sm font-medium">API Management</span>
              </div>
              ...
              <p className="text-slate-400 mt-2 text-sm md:text-base">Manage your API keys and monitor usage</p>
            </div>
            <button
      ...
      <PremiumCard className="p-4 md:p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Your organization's API access tokens</p>
          </div>
        </div>
        ...
        {apiKeys.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h4 className="text-lg font-semibold text-slate-900 mb-2">No API Keys Yet</h4>
            <p className="text-slate-500 mb-4">Create your first API key to start integrating with our API.</p>
            <button
              onClick={() => setShowCreateModal(true)}
            ...
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Key Prefix</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Scopes</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Last Used</th>
                ...
                {apiKeys.map((key) => (
                  <tr key={key.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <td className="py-4 px-4">
                      ...
                      <div className="text-xs text-slate-500 mt-0.5">Rate: {key.rateLimitPerMin}/min</div>
                    </td>
                    <td className="py-4 px-4">
                    ...
                    <td className="py-4 px-4">
                      <div className="relative">
                        <button
                          onClick={() => setShowScopes(showScopes === key.id ? null : key.id)}
                          className="flex items-center gap-1 text-sm text-slate-600 hover:text-slate-900"
                        >
                          {showScopes === key.id ? <EyeOff size={14} /> : <Eye size={14} />}
                          {(key.scopes as string[]).includes('*') ? 'Full Access' : `${(key.scopes as string[]).length} scopes`}
                        </button>
                        {showScopes === key.id && (
                          <div className="absolute z-10 mt-2 p-3 bg-white rounded-lg shadow-xl border border-slate-200 min-w-[200px]">
                            ...
                            <div className="space-y-1">
                              {(key.scopes as string[]).map((scope) => (
                                <div key={scope} className="text-sm text-slate-700 flex items-center gap-2">
      ...
      <PremiumCard className="p-4 md:p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Current API rate limiting configuration</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-slate-50 rounded-lg">
            ...
            <div className="text-sm text-slate-600 mt-1">Requests per minute (default)</div>
          </div>
          <div className="p-4 bg-slate-50 rounded-lg">
        ...
        <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
          <p className="text-sm text-blue-800">
            <strong>Need higher limits?</strong> Upgrade your plan or contact support for enterprise rate limits.
          </p>
        </div>
      </PremiumCard>
      ...
      <PremiumCard className="p-4 md:p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Recent API Requests</h3>
              <p className="text-sm text-slate-500">Latest API activity across all keys</p>
            </div>
          </div>
          <button
            onClick={fetchData}
            ...
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Endpoint</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Response Time</th>
                ...
                {logs.map((log) => (
                  <tr key={log.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <td className="py-3 px-4">
                      <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${getMethodColor(log.method)}`}>
                        {log.method}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <code className="text-sm text-slate-700">{log.endpoint}</code>
                    </td>
                    <td className="py-3 px-4">
              ...
              {createdKey ? (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-slate-700">Your API Key</label>
                    <div className="flex items-center gap-2 p-3 bg-slate-900 rounded-lg">
                      <code className="flex-1 text-sm font-mono text-emerald-400 break-all">{createdKey}</code>
                      <button
                ...
                <div className="space-y-5">
                  ...
                  <div>
                      ...
                      {AVAILABLE_SCOPES.map((scope) => (
                        <label
                          ...
                          className={`flex items-start gap-3 p-2 rounded-lg cursor-pointer transition-colors ${
                            newKeyScopes.includes(scope.id) ? 'bg-orange-50 border border-orange-200' : 'hover:bg-slate-100'
                          }`}
                        >
                          <input
                            type="checkbox"
                            checked={newKeyScopes.includes(scope.id) || (scope.id !== '*' && newKeyScopes.includes('*'))}
                            onChange={() => toggleScope(scope.id)}
                            disabled={scope.id !== '*' && newKeyScopes.includes('*')}
                            className="mt-0.5 rounded border-slate-300 text-orange-500 focus:ring-orange-500"
                          />
                          <div>
                            <div className="text-sm font-medium text-slate-900">{scope.label}</div>
                    ...
                    {newKeyScopes.length === 0 && (
                      <p className="text-xs text-slate-500 mt-2">No scopes selected - key will have full access by default</p>
                    )}
                  </div>
