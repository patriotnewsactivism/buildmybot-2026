...
const MOCK_LANDING_PAGES: LandingPage[] = [
  {
    ...
    subheadline: 'Join thousands of professionals who have transformed their productivity with our AI-powered solution.',
    ...
    seoTitle: 'Product Launch 2024 - Early Access',
    seoDescription: 'Be the first to experience our revolutionary AI-powered solution.',
    botId: 'bot1',
    isPublished: true,
    viewCount: 1247,
    conversionCount: 89,
  ...
  },
  {
    ...
    subheadline: 'Discover how our experts can help grow your business.',
    ctaText: 'Schedule Now',
    ctaColor: '#3B82F6',
    formFields: [
      { id: 'f1', type: 'text', label: 'Name', placeholder: 'Your name', required: true },
      { id: 'f2', type: 'email', label: 'Email', placeholder: 'email@example.com', required: true },
      { id: 'f3', type: 'textarea', label: 'Tell us about your project', placeholder: 'Describe your goals...', required: false },
    ],
...
const DEFAULT_FORM_FIELDS: FormField[] = [
  { id: 'default-name', type: 'text', label: 'Full Name', placeholder: 'Enter your name', required: true },
  { id: 'default-email', type: 'email', label: 'Email Address', placeholder: 'you@example.com', required: true },
];
...
export const LandingPageBuilder: React.FC<LandingPageBuilderProps> = ({ bots }) => {
  const [pages, setPages] = useState<LandingPage[]>(MOCK_LANDING_PAGES);
  const [view, setView] = useState<'list' | 'editor'>('list');
  const [selectedPage, setSelectedPage] = useState<LandingPage | null>(null);
  const [activeEditorTab, setActiveEditorTab] = useState<'basic' | 'hero' | 'cta' | 'form' | 'thankyou' | 'bot'>('basic');
  ...
  const handleTogglePublish = (pageId: string) => {
    setPages(pages.map(p =>
      p.id === pageId ? { ...p, isPublished: !p.isPublished, updatedAt: new Date().toISOString() } : p
    ));
  };
  ...
  const handleSave = async () => {
    ...
    const updatedPage = { ...selectedPage, updatedAt: new Date().toISOString() };
    const existingIndex = pages.findIndex(p => p.id === selectedPage.id);
  ...
  const updateSelectedPage = <K extends keyof LandingPage>(field: K, value: LandingPage[K]) => {
    if (selectedPage) {
  ...
  const handleAddFormField = (type: FormField['type']) => {
    ...
    const newField: FormField = {
      id: `field-${Date.now()}`,
      type,
      label: type === 'checkbox' ? 'I agree to the terms' : `New ${type.charAt(0).toUpperCase() + type.slice(1)} Field`,
  ...
  const handleRemoveFormField = (fieldId: string) => {
    if (!selectedPage) return;
    updateSelectedPage('formFields', selectedPage.formFields.filter(f => f.id !== fieldId));
  };

  const handleUpdateFormField = (fieldId: string, updates: Partial<FormField>) => {
    if (!selectedPage) return;
    updateSelectedPage('formFields', selectedPage.formFields.map(f =>
  ...
  const handleDragOver = (e: React.DragEvent, targetFieldId: string) => {
    e.preventDefault();
    if (!selectedPage || !draggedFieldId || draggedFieldId === targetFieldId) return;
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-xl md:rounded-2xl p-4 md:p-8">
            ...
            <div className="relative z-10">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-emerald-400 text-sm font-medium">Lead Capture</span>
              </div>
                ...
                <PremiumCard key={page.id} className="overflow-hidden">
                  <div className="p-4 md:p-6">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      >
                        {page.isPublished ? <ToggleRight size={28} /> : <ToggleLeft size={28} />}
                      </button>
                    </div>
                    ...
                    {bot && (
                      <div className="flex items-center gap-2 mb-4 p-2 bg-slate-50 rounded-lg">
                        ...
                        <span className="text-sm text-slate-600 truncate">{bot.name}</span>
                      </div>
                    )}

                    <div className="grid grid-cols-3 gap-3 mb-4">
                      <div className="text-center p-2 bg-slate-50 rounded-lg">
                        <div className="text-lg font-bold text-slate-900">{page.viewCount.toLocaleString()}</div>
                        <div className="text-xs text-slate-500">Views</div>
                      </div>
                      <div className="text-center p-2 bg-slate-50 rounded-lg">
                        <div className="text-lg font-bold text-slate-900">{page.conversionCount}</div>
                        <div className="text-xs text-slate-500">Leads</div>
                      </div>
                      <div className="text-center p-2 bg-slate-50 rounded-lg">
                        <div className="text-lg font-bold text-orange-600">{getConversionRate(page)}%</div>
                  ...
                  <div className="px-4 md:px-6 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${page.isPublished ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
            ...
            {filteredPages.length === 0 && (
              <div className="col-span-full">
                <PremiumCard className="p-12 text-center">
                  ...
                  <h4 className="font-semibold text-slate-900 mb-2">No landing pages found</h4>
                  <p className="text-slate-500 text-sm mb-4">Create your first landing page to start capturing leads.</p>
                  <button
                    onClick={handleCreateNew}
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-500">
                    <span className={`w-2 h-2 rounded-full ${selectedPage.isPublished ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50/50">
                ...
                <div className="space-y-6 animate-fade-in">
                  <div className="bg-white p-6 rounded-xl border border-slate-200">
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                      <FileText size={18} className="text-orange-500" /> Basic Information
                    </h3>
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            onChange={(e) => updateSelectedPage('slug', e.target.value.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, ''))}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateSelectedPage('seoTitle', e.target.value)}
                          className="w-full rounded-lg border-slate-200 focus:ring-orange-500 focus:border-orange-500"
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) => updateSelectedPage('seoDescription', e.target.value)}
              ...
              {activeEditorTab === 'hero' && (
                <div className="space-y-6 animate-fade-in">
                  <div className="bg-white p-6 rounded-xl border border-slate-200">
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                      <Type size={18} className="text-orange-500" /> Hero Content
                    </h3>
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateSelectedPage('headline', e.target.value)}
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) => updateSelectedPage('subheadline', e.target.value)}
                  ...
                  <div className="bg-white p-6 rounded-xl border border-slate-200">
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                      <ImageIcon size={18} className="text-orange-500" /> Hero Image
                    </h3>
                    {selectedPage.heroImageUrl ? (
                      <div className="relative group">
                        ...
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition rounded-lg flex items-center justify-center gap-3">
                          ...
                          <button
                            onClick={() => updateSelectedPage('heroImageUrl', undefined)}
              ...
              {activeEditorTab === 'cta' && (
                <div className="space-y-6 animate-fade-in">
                  <div className="bg-white p-6 rounded-xl border border-slate-200">
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                      <MousePointer size={18} className="text-orange-500" /> Call to Action Button
                    </h3>
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateSelectedPage('ctaText', e.target.value)}
                          className="w-full rounded-lg border-slate-200 focus:ring-orange-500 focus:border-orange-500"
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <input
                              ...
                              onChange={(e) => updateSelectedPage('ctaColor', e.target.value)}
                  ...
                  <div className="bg-white p-6 rounded-xl border border-slate-200">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-bold text-slate-800 flex items-center gap-2">
                        <FormInput size={18} className="text-orange-500" /> Form Fields
                      </h3>
                    </div>
                    ...
                    <div className="space-y-3 mb-6">
                      {selectedPage.formFields.map((field) => (
                        ...
                        >
                          <GripVertical className="text-slate-400 mt-2 flex-shrink-0" size={16} />
                          <div className="flex-1 space-y-3">
                            <div className="flex items-center gap-3">
                              ...
                              <select
                                value={field.type}
                                onChange={(e) => handleUpdateFormField(field.id, { type: e.target.value as FormField['type'] })}
                      ...
                      {selectedPage.formFields.length === 0 && (
                        <div className="text-center py-8 text-slate-400">
                          ...
                          <p className="text-sm">No form fields yet. Add fields below.</p>
                        </div>
                      )}
                    </div>

                    <div className="border-t border-slate-200 pt-4">
                      <label className="block text-sm font-medium text-slate-700 mb-3">Add Field</label>
                      <div className="flex flex-wrap gap-2">
                        {(['text', 'email', 'phone', 'textarea', 'checkbox'] as FormField['type'][]).map(type => (
                          <button
                            key={type}
              ...
              {activeEditorTab === 'thankyou' && (
                <div className="space-y-6 animate-fade-in">
                  <div className="bg-white p-6 rounded-xl border border-slate-200">
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                      <CheckCircle size={18} className="text-orange-500" /> Thank You Message
                    </h3>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-2">Message after form submission</label>
                        <textarea
                          value={selectedPage.thankYouMessage || ''}
                          onChange={(e) => updateSelectedPage('thankYouMessage', e.target.value)}
                      ...
                      <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                        ...
                        <p className="text-emerald-600 text-sm">
                          {selectedPage.thankYouMessage || 'Your thank you message will appear here.'}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {activeEditorTab === 'bot' && (
                <div className="space-y-6 animate-fade-in">
                  <div className="bg-white p-6 rounded-xl border border-slate-200">
                    <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                      <BotIcon size={18} className="text-orange-500" /> Bot Integration
                    </h3>
                    <div className="space-y-4">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => updateSelectedPage('botId', e.target.value || undefined)}
                          className="w-full rounded-lg border-slate-200 focus:ring-orange-500 focus:border-orange-500"
                        >
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                          The selected bot will power the chat widget on this landing page.
                        </p>
                      </div>
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-medium text-slate-800">{bot.name}</div>
                                  <div className="text-xs text-slate-500">{bot.type} • {bot.model}</div>
                                </div>
                              </div>
                            );
                          })()}
              ...
              <div className={`bg-white shadow-xl transition-all duration-300 overflow-hidden ${previewMode === 'mobile' ? 'w-[375px] rounded-3xl border-8 border-slate-800' : 'w-full rounded-lg'}`}>
                <div className="flex flex-col min-h-full">
                  <nav className="p-4 border-b border-gray-100 flex justify-between items-center">
                    <span className="font-bold text-lg text-gray-800">{selectedPage.name}</span>
                  ...
                  <header className="relative py-12 md:py-16 px-6 text-center bg-gradient-to-b from-slate-50 to-white overflow-hidden">
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-base md:text-lg text-gray-600 max-w-lg mx-auto mb-8">
                        {selectedPage.subheadline || 'Your subheadline will appear here.'}
                      </p>
                    </div>
                  </header>
                  ...
                  {selectedPage.botId && (
                    <div className="fixed bottom-4 right-4 z-10">
                      <div
                        ...
                        style={{ backgroundColor: getAssociatedBot(selectedPage.botId)?.themeColor || '#3B82F6' }}
                      >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-slate-600"><strong>{selectedPage.viewCount.toLocaleString()}</strong> views</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <CheckCircle size={16} className="text-emerald-500" />
                    <span className="text-slate-600"><strong>{selectedPage.conversionCount}</strong> conversions</span>
                  </div>
                </div>
