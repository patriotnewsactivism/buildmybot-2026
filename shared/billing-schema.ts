import { pgTable, text, varchar, integer, boolean, timestamp, json, real, pgEnum } from 'drizzle-orm/pg-core';
import { relations } from 'drizzle-orm';
import { organizations, users, bots, botTemplates, supportTickets } from './schema';
...
export const usageResourceEnum = pgEnum('usage_resource', ['voice_minutes', 'sms_credits', 'email_credits', 'storage_mb', 'api_calls']);
export const serviceTypeEnum = pgEnum('service_type', ['onboarding', 'training', 'kb_import', 'template', 'landing_page', 'custom']);
export const orderStatusEnum = pgEnum('order_status', ['pending', 'paid', 'processing', 'completed', 'refunded', 'canceled']);
...
export const serviceOrders = pgTable('service_orders', {
  ...
  serviceId: text('service_id').notNull().references(() => serviceOfferings.id),
...
export const templatePurchases = pgTable('template_purchases', {
  ...
  organizationId: text('organization_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
  userId: text('user_id').references(() => users.id),
  templateId: text('template_id').notNull().references(() => botTemplates.id),
  pricePaidCents: integer('price_paid_cents'),
  stripePaymentIntentId: text('stripe_payment_intent_id'),
  createdAt: timestamp('created_at').defaultNow(),
});
...
export const voiceMinutesPackagesRelations = relations(voiceMinutesPackages, ({}) => ({}));
...
export const serviceOfferingsRelations = relations(serviceOfferings, ({ many }) => ({
...
export const analyticsDailyMetricsRelations = relations(analyticsDailyMetrics, ({ one }) => ({
  organization: one(organizations, {
...
export type InsertVoiceMinutesPackage = typeof voiceMinutesPackages.$inferInsert;
